...
public class ProjectEditorWindow extends JFrame {
	...
	private List<CombinedSprite> stripes;
	...
	public void managedSpriteSelected(boolean val) {
		...
		UIUtils.setPanelEnabled(luaPanel, val);
	...
	public ProjectEditorWindow() {
		...
		gbl_managedSpriteDetails.columnWidths = new int[] { 377, 0 };
		...
		spriteTypeComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (e.getActionCommand().equals("comboBoxChanged")) {
					if (spriteTypeComboBox.getSelectedItem() == null)
						return;
		...
		lblFilePattern.setToolTipText(
				"<html>For files with filename myRenderoutputfile001.png the corresponding pattern would be <br/><br/>myRenderoutputfile%03d.png<br/><br/><br/>%d = any integer and %03d = any ineger padded to length 3 with zeros.<br/><br/><br/>The number of leading zeros must be specified for fiel detection to work. only integers can currently be used.</html>");
		...
		btnGenerateStripes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (tiles == null)
					UIUtils.showError(new Exception("No tiles loaded"), ProjectEditorWindow.this);
				SpriteCombinator sc = new SpriteCombinator(selectedSprite.getType(), tiles, ProjectEditorWindow.this);
		...
		gbc_tilesPanel.insets = new Insets(0, 5, 5, 5);
		...
		tilesList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent arg0) {
				if (tilesList.getSelectedValue() == null)
					return;
				try {
		...
		luaPanel = new JPanel();
		luaPanel.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null));
		GridBagConstraints gbc_luaPanel = new GridBagConstraints();
		gbc_luaPanel.fill = GridBagConstraints.BOTH;
		gbc_luaPanel.gridheight = 3;
		gbc_luaPanel.insets = new Insets(0, 0, 5, 0);
		gbc_luaPanel.gridx = 3;
		gbc_luaPanel.gridy = 0;
		contentPane.add(luaPanel, gbc_luaPanel);
		GridBagLayout gbl_luaPanel = new GridBagLayout();
		gbl_luaPanel.columnWidths = new int[]{337, 0};
		gbl_luaPanel.rowHeights = new int[]{29, 0, 0};
		gbl_luaPanel.columnWeights = new double[]{1.0, Double.MIN_VALUE};
		gbl_luaPanel.rowWeights = new double[]{0.0, 1.0, Double.MIN_VALUE};
		luaPanel.setLayout(gbl_luaPanel);
		...
		gbc_panel_8.gridx = 0;
		gbc_panel_8.gridy = 0;
		luaPanel.add(panel_8, gbc_panel_8);
		
		JLabel lblLuagfxlib = new JLabel("LuaGFXLib");
		panel_8.add(lblLuagfxlib);
		...
		stripesList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent arg0) {
				
				if (stripesList.getSelectedValue() == null)
					return;
				try {
					spriteViewerPanel.setImage(ImageIO.read(stripesList.getSelectedValue().getFile()));
				} catch (IOException e) {
					UIUtils.showError(e, ProjectEditorWindow.this);
					e.printStackTrace();
				}
		...
		stripesList.setModel(new DefaultListModel<CombinedSprite>());
		SpriteListCellRenderer flcr1 = new SpriteListCellRenderer();
		stripesList.setCellRenderer(flcr1);
		...
		gbc_tileViewPanel.insets = new Insets(0, 0, 0, 5);
